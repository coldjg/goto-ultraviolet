...
/// Confirm.js
winloc = window.location; searchparam = new URLSearchParams(winloc.search);if(confirm("*notices error* OwO what's this? A page that is blocked for being too "+ searchparam.get("CAT") +"? *Translates url to ultraviolet xor cutely* U- UwU master here's your unblocked webpage nyaaaaaaaaa~~! pls accept my offering master " + searchparam.get("ACC") + "!")){(function(){url = searchparam.has('RAW') ? searchparam.get("RAW") : winloc.href.toString(); "http" != url.substring(0, 4) && (url = "https://" + url); winloc.href = "https://reallypurple-node-production.up.railway.app/uv/service/" + encodeURIComponent(url.toString().split("").map((char, ind) => ind % 2 ? String.fromCharCode(char.charCodeAt() ^ 2) : char).join(""));})();}
